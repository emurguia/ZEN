...
let rec string_of_sexpr (t, e) =
  ...
  | STupleLit(e1, e2) -> "(" ^ string_of_sexpr e1 ^ ", " ^ string_of_sexpr e2 ^ ")"
  | SId(s) -> s
  | SBinop(e1, o, e2) ->
      string_of_sexpr e1 ^ " " ^ string_of_op o ^ " " ^ string_of_sexpr e2
  | SUnop(o, e) -> string_of_uop o ^ string_of_ssexpr e
  | SAssign(v, e) -> v ^ " = " ^ string_of_sexpr e
  | SListAccess(s, e) -> s ^ "[" ^ string_of_sexpr e ^ "]"
  | SListAssign(s, e1, e2) -> s ^ "[" ^ string_of_sexpr e1 ^ "] = " ^ string_of_expr e2
  | SCall(f, el) ->
